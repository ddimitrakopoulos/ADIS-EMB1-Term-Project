...
import netlsd # for wave kernel (not karate club tho, but PyPI)
...
def generate_netlsd_embeddings_wave(graphs, dim=250, kernel="heat"):
    ...
    for g in graphs:
        ...
        if kernel == "heat":
            emb = netlsd.heat(g, timescales=np.logspace(-2, 2, dim))
        elif kernel == "wave":
            emb = netlsd.wave(g, timescales=np.logspace(-2, 2, dim))
        else:
