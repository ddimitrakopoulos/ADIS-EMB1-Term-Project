...
def run_stability(graphs, X, clf, y, train_idx, test_idx, dim, seed, 
                  permute_pct=0.1, mode='both', orig_acc=None, attributed=False):
    ...
    perturbed = [perturb_graph(G, edge_perturb_ratio=permute_pct, mode=mode) for G in graphs]

    g2v = Graph2Vec(dimensions=dim, wl_iterations=wl_iterations, epochs=epochs, learning_rate=learning_rate, seed=env_seed, attributed=attributed)
    g2v.fit(perturbed)
...
def run_full_stability_analysis(graphs, X, clf, y, train_idx, test_idx, dim, seed, orig_acc, attributed=False):
    ...
    for pct in perturbation_levels:
        if pct == 0.0:
            ...
            result = run_stability(
                ...
                permute_pct=pct, mode='add', orig_acc=orig_acc, attributed=attributed
            )
    ...
    for pct in perturbation_levels:
        if pct == 0.0:
            ...
            result = run_stability(
                graphs, X, clf, y, train_idx, test_idx, dim, seed,
                permute_pct=pct, mode='remove', orig_acc=orig_acc, attributed=attributed
            )
            results.append(result)
    ...
    for pct in perturbation_levels:
        if pct == 0.0:
            ...
            result = run_stability(
                graphs, X, clf, y, train_idx, test_idx, dim, seed,
                permute_pct=pct, mode='both', orig_acc=orig_acc, attributed=attributed
            )
            results.append(result)
    ...
    for pct in perturbation_levels:
        if pct == 0.0:
            ...
            result = run_stability(
                graphs, X, clf, y, train_idx, test_idx, dim, seed,
                permute_pct=pct, mode='shuffle', orig_acc=orig_acc, attributed=attributed
            )
            results.append(result)
    ...
    ax1.plot(pct_vals, emb_curves['both'], marker='D', color='orange', linewidth=2, label='Add+Drop Edges')
    ...
    ax2.plot(pct_vals, acc_curves['both'], marker='D', color='orange', linewidth=2, label='Add+Drop Edges')
